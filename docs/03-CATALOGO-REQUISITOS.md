...
| RNF02 | Política de contraseñas, bloqueo, expiración de sesión, JWT 8 h, 2FA 5 min | 🟡 | **Cumple:** JWT de 8 h, 2FA de 5 min, bloqueo escalado (1/5/15/30/60 min) y, desde el 2-09-2026, **política de contraseñas en el servidor** (`utils/password.js`) y **recuperación de contraseña** operativa. Antes la API aceptaba la contraseña `"1"`. Desde el 3-09-2026 cumple además el **cierre por 30 min de inactividad** (`useCierrePorInactividad.js`, en el navegador) y la política exige **carácter especial**, que HU-01.6 pedía y no se comprobaba. **Sigue sin cumplir:** falta un limitador dedicado en `/api/auth/login` |
